...
for dtype in ['host_genome', 'serology', 'methylome']:
    filtered_df = metadata_df[(metadata_df['data_type'] == dtype) & 
                               ~(metadata_df['IntervalName'].str.startswith('Screening')) & 
                               ~(metadata_df['IntervalName'].str.startswith('Additional'))]
...
print
print "Total blood samples: %s" % len(all_biopsy_ids)